...
pub struct AclTreeNode {
    /// [User](pdm_api_types::User) or
    /// [Token](pdm_api_types::ApiToken) ACLs for this node.
...
impl AclTreeNode {
    ...
    /// [Authid](pdm_api_types::Authid).
    ///
    /// If the `Authid` is a [User](pdm_api_types::User) that has no specific `Roles` configured on
...
pub const ACL_CFG_FILENAME: &str = "/etc/proxmox-datacenter-manager/access/acl.cfg";
/// Path used to lock the [`AclTree`] when modifying.
pub const ACL_CFG_LOCKFILE: &str = "/etc/proxmox-datacenter-manager/access/.acl.lock";

/// Get exclusive lock
pub fn lock_config() -> Result<ApiLockGuard, Error> {
    open_api_lockfile(ACL_CFG_LOCKFILE, None, true)
}
