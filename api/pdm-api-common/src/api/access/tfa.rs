...
fn list_tfa(rpcenv: &mut dyn RpcEnvironment) -> Result<Vec<methods::TfaUser>, Error> {
    ...
    let _lock = crate::auth::tfa::read_lock()?;
    let tfa_data = crate::auth::tfa::read()?;
...
fn list_user_tfa(userid: Userid) -> Result<Vec<methods::TypedTfaInfo>, Error> {
    ...
    methods::list_user_tfa(&crate::auth::tfa::read()?, userid.as_str())
}
...
fn get_tfa_entry(userid: Userid, id: String) -> Result<methods::TypedTfaInfo, Error> {
    ...
    match methods::get_tfa_entry(&crate::auth::tfa::read()?, userid.as_str(), &id) {
        Some(entry) => Ok(entry),
...
async fn delete_tfa(
    ...
    let mut data = crate::auth::tfa::read()?;
