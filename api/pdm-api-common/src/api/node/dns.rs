...
pub fn read_etc_resolv_conf(expected_digest: Option<&str>) -> Result<Value, Error> {
    let mut result = json!({});
    ...
    result["digest"] = Value::from(hex::encode(digest));
...
pub fn update_dns(
    ...
    let mut config = read_etc_resolv_conf(digest.as_deref())?;
