...
const METRIC_POLL_INTERVAL: u64 = 15 * 60; // once every 15 minutes
...
async fn resource_caching_task() -> Result<(), Error> {
    loop {
        if let Err(err) =
            crate::api::resources::get_resources_impl(METRIC_POLL_INTERVAL, None, None, None, None)
                .await
        {
            log::error!("could not update resource cache: {err}");
        }

        let delay_target = task_utils::next_aligned_instant(METRIC_POLL_INTERVAL + 10);
