...
use pbs_api_types::userid::PROXMOX_TOKEN_NAME_FORMAT;
        ...
        let (authid, token) = match entry.ty {
            RemoteType::Pve => {
                ...
                let token = client
                    ...
                    .await
                    .map_err(|err| match std::error::Error::source(&err) {
                        Some(err) => format_err!("error creating token: {err}"),
                        None => format_err!("error creating token: {err}"),
                    })?;
